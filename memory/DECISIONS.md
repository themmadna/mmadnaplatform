...
Decision: Added `--post-event` argparse flag plus a separate workflow `post-event-scraper.yml` running `0 */2 * * *`. Guarded by `is_post_event_window()` (event `start_time + 5h` to `start_time + 48h`, fails safe on NULL). Runs **all phases 0–6**.
Reasoning: Live-mode scraping covers the event window but post-event metadata, judge scorecards, and stats fill-ins continue arriving for ~48h. A separate slower cadence avoids hammering UFCStats/mmadecisions while still closing data gaps without manual triggering. Two-hour interval is enough to catch mmadecisions publishes (which trickle in over Sunday–Monday). Phases 2/3/4 are included (not just `--live`'s job) so any completed-fight results, metadata, or round stats missed during the live window — ufcstats posting late, or a replaced-matchup bout `--live` couldn't match — are still caught.
...
Note (2026-05-30): This entry originally read "runs Phases 0/0.5/1/5/6, skips 2/3/4." That described the flag as first shipped (2026-04-25, commit 8b9d0915). Phases 2/3/4 were added the same day in commit 327bd5b8 to close the post-event stats gap; the entry was stale from when it was written and is now corrected to match the code.
...
## [2026-05-30] — Event "is over" signal keyed off the main event + opponent-swap resolution in the poller
Decision: An event is considered concluded when its MAIN EVENT (lowest `card_position`) finalizes, surfaced via a new `ufc_events.ended_at` column. `poll-live-fights` stamps it; a scraper-side `stamp_event_ended_at` and a frontend `start_time + 8h` window are backstops. `isLiveEvent` returns false on `ended_at`, and the fights view hides never-started bouts once concluded. Separately, `poll-live-fights` resolves late opponent swaps: an UNCLAIMED ESPN competition that shares exactly one fighter with an unmatched fight is treated as a confident match, re-linking the comp id and rewriting the `bout` to ESPN's fighters.
Reasoning: The LIVE badge was purely time-based (LIVE until local midnight). "All fights ended" can't be the event-over signal because scratched/replaced bouts never reach FINAL, so it would never trip — the main event always fights last, making it the reliable terminal event. For swaps: ESPN re-creates the bout under a new competition id with one fighter changed, defeating both the id match and the both-fighters name match; a fighter is unique within an event, so one shared fighter is unambiguous. Rewriting the bout (not just re-linking) lets the post-event ufcstats scrape update the row IN PLACE rather than inserting a duplicate completed row. Verified live on UFC Fight Night: Song vs Figueiredo (2026-05-30) — resolved all 3 swapped rows within one cron cycle, no duplicates, no manual deletion.
Alternatives considered: Frontend-hide-only (hide never-started bouts, don't track them live) — rejected as the sole fix because swapped bouts stay untracked/unscorable during the event and the data stays wrong; kept as a backstop. "All fights ended" as the event-over signal — rejected (never trips with scratched bouts). Time-based LIVE cap alone — rejected as primary (fragile across card lengths and early stoppages); kept only as the 8h safety net.
