...
- **The frontend "event is over" signal must key off the main event, never "all fights ended."** `isLiveEvent()` was purely time-based (`event_date===today && start_time<=now`), so an event showed LIVE until local midnight. The instinct to fix it with "are all fights done?" fails: scratched/replaced bouts never reach FINAL, so that check never trips. The robust signal is the main event (lowest `card_position`) finalizing — it always fights last, so once it ends the card is over. Implemented as `ufc_events.ended_at`, stamped by `poll-live-fights` and checked by `isLiveEvent`. Same `ended_at` (or `eventFights[0].fight_ended_at`) gates the fights view to hide never-started bouts so phantom "Upcoming" cards disappear without waiting on DB cleanup.
