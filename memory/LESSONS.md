...
- **Add an index for the join you have, not the join you plan to have.** `idx_round_fight_stats_fight_url` (S-P2-13) was deliberately deferred behind S-P2-11: until the view refactor actually switched `fight_dna_metrics` to aggregate on `fight_url`, nothing queried `rfs.fight_url` as a join key and the index would have been dead weight (Postgres maintains it on every write for zero read benefit). The REMEDIATION-PLAN encoded this as a hard dependency chain (#5 backfill → #11 view → #13 index). **How to apply:** when an audit flags a "missing index," confirm a live query path actually uses that column as a predicate/join key first — `pg_stat_user_indexes.idx_scan` on similar indexes, or grep the view/RPC/trigger SQL. An index with no consuming query is pure write-amplification.
- **`CREATE INDEX IF NOT EXISTS` is idempotent but says nothing about whether it ran — verify against `pg_indexes` after.** The Management API returns 201 for a no-op `IF NOT EXISTS` just as it does for a real create. `deploy_indexes.py` now post-verifies the full expected set against `pg_indexes` and exits non-zero on any miss, so a typo'd index name or a silently-skipped statement can't pass as success.
