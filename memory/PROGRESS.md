...
Next session: (1) Opponent-swap duplicate from the backfill — merge 8838 "Ding Meng vs Jose Henrique" (orphan upcoming, ESPN ts+comp_id) into 8856 "Jose Souza vs Ding Meng" (completed, result); verify premise first, destructive, needs approval. (2) Resume Phase C — S-P2-13 indexes, then S-P2-8 search_path hardening / S-P2-9 anon EXECUTE revokes. Also: scraper fix awaiting commit+push approval.
...
- **NextSteps:** (1) **Opponent-swap duplicate** surfaced by the backfill — id 8838 "Ding Meng vs Jose Henrique" (orphan, still `upcoming`, carries ESPN `fight_started/ended_at` + `comp_id 401871900`) duplicates id 8856 "Jose Souza vs Ding Meng" (`completed`, Decision-Split, Jose Souza, no timestamps). Same physical fight; Phase 2 alias-matcher didn't recognize the new opponent name → inserted a 2nd row. Verify name-variant vs genuine swap against ufcstats/ESPN, then single-tx merge (fold timestamps+comp_id into the completed row, delete the orphan) — destructive, needs approval. Note: id 8836 Alex Perez vs Sumudaerji "Could Not Continue" / `winner=NULL` is CORRECT (genuine no-result), no fix. (2) Scraper fix awaiting **commit + push approval** (Python-only, no Vercel impact; restores the crash-looping GitHub Actions workflows). (3) Then resume Phase C.
