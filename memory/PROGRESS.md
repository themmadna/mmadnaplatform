...
**Phase C — S-P2-9 resolved (2026-05-31) — Checkpoint**
- **Goal:** Close S-P2-9 — revoke `anon` + `PUBLIC` EXECUTE on the 4 user-scoped RPCs so the grant surface matches the docs (all key on `auth.uid()`; today they return empty for anon, but the wide grant is a latent risk if a NULL-as-match-all bug ever lands).
- **Constraints:** Must keep `authenticated` access intact (verify each fn has it before revoking); bake REVOKE into the version-controlled deploy scripts after the GRANT; confirm no guest frontend path breaks.
- **Progress:** Pre-flight confirmed all 4 had `anon, authenticated, PUBLIC, postgres, service_role` EXECUTE — so revoking anon/PUBLIC is safe (authenticated retained). 3 live fns: added `REVOKE EXECUTE ... FROM anon, PUBLIC` after `GRANT ... TO authenticated` in `deploy_judging_profile`/`deploy_user_judge_comparison`/`deploy_scoring_insights` (judging_profile had no explicit GRANT — added one) + redeployed. 4th fn `get_liked_fight_stats`: reachability triad (grep `src/` + `pg_proc.prosrc` + `information_schema.views`) returned **zero references** → dead; also orphan (no deploy script) + Convention #9 `(event_name, bout)` join → applied REVOKE ad-hoc and filed S-P2-19 to DROP it. Frontend: `getUserJudgeComparison` was the one dataService wrapper missing the `if (!user) return null` guard its 2 siblings have — added it so guests don't fire a doomed anon call (anon now errors instead of returning empty). `npm run build` clean (+7 B). Post-verify: all 4 fns grant EXECUTE to `authenticated, postgres, service_role` only.
- **Decisions:** (a) Didn't reconstruct a deploy script for `get_liked_fight_stats` — version-controlling dead, buggy code just to host a revoke blesses the bug; ad-hoc REVOKE + DROP follow-up is cleaner. (b) Treated the backend revoke and the frontend guard as one change (a tightened grant turns an unguarded guest call from empty→error; the call sites must guard). (c) Kept `get_community_scorecard` anon-accessible (aggregate-only) — correctly excluded from this set.
- **NextSteps:** Phase C remaining P2s — S-P2-10 (drop redundant `fight_ratings` "Public Read Ratings" SELECT policy, keep "Enable read access for all users"), S-P2-12 (add `fight_meta_details.fight_url → fights.fight_url ON DELETE CASCADE` FK), S-P2-14 (revoke EXECUTE on `update_fight_ratings()` from PUBLIC/anon/authenticated — trigger machinery doesn't need it). Plus new S-P2-19 (DROP dead `get_liked_fight_stats` — destructive, needs approval).
...
- [x] **S-P2-9.** ~~Revoke anon EXECUTE on 4 user-scoped RPCs~~ — **RESOLVED 2026-05-31**. Baked `REVOKE ... FROM anon, PUBLIC` (after `GRANT ... TO authenticated`) into the 3 live fns' deploy scripts (`deploy_judging_profile`, `deploy_user_judge_comparison`, `deploy_scoring_insights`) + redeployed. 4th fn `get_liked_fight_stats` is dead + orphan + buggy → REVOKE applied ad-hoc, flagged for DROP as **S-P2-19**. Added `if (!user) return null` guard to `getUserJudgeComparison` in `dataService.js` (siblings already had it). Verified: all 4 now `authenticated, postgres, service_role` only. Build clean (+7 B).
...
**Data Pipeline Outage — GitHub Actions auto-disabled — 2026-09-05 — Checkpoint**
- **Goal:** Bastian asked why the last event in the DB was 2026-08-08 when several events had happened since. Diagnose and fix.
- **Constraints:** Verify the root cause before touching the scrapers (they were the obvious suspect and were innocent); snapshot before any run that can delete; verify any auto-delete against ufcstats before accepting it.
- **Progress:** Root cause = **both scraper workflows in GitHub state `disabled_inactivity`**. GitHub disables scheduled workflows after 60 days without repo activity; last commit 2026-06-09 → disabled 2026-08-09 (+60d exactly), mid-way through the Gamrot vs Salkilld card. Scrapers themselves were fine — the ufcstats PoW solver still clears the challenge first try. **Recovered:** full pipeline run backfilled 4 missing events (UFC 330 Makhachev vs Machado Garry 8/15, Hernandez vs Rodrigues 8/22, Nurmagomedov vs Song 8/29, Hooker vs Parnasse 9/5) + Noche UFC 9/12 as upcoming — 5 new events, 60 new fights, 51 meta, 224 round rows, judge scorecards for all. Repaired the Gamrot card (4 fights stuck `upcoming` → completed; meta 8→12, rounds 34→50). A `--live` run then picked up tonight's in-flight card and auto-deleted stale row 8950 (Sutherland vs Da Silva Lopes) — **verified against ufcstats before accepting**: that card lists 12 bouts, Sutherland's real opponent was Jose Montanha, so it was a genuine pre-event opponent swap and the delete was correct. Every past event now reads fights == completed == winners == meta. **Code fixes:** `.github/workflows/keepalive.yml` (monthly empty `[skip ci]` commit, resets the 60-day timer) + Phase 5 widened to backfill `start_time` on past events up to `PHASE5_BACKFILL_DAYS`=45 back when it is still NULL.
- **Decisions:** (a) Fixed Phase 5's future-only window rather than one-off patching the 4 events — a NULL `start_time` is self-perpetuating, since `is_live_window()`/`is_post_event_window()` both fail safe on NULL and would keep those events invisible to automation forever. (b) Corrected my own mid-session claim that NULL `start_time` left voting open on past fights — `isVotingLocked` means "voting opens at event start," so a past event is unlocked either way. No voting bug existed; the real consequence was automation invisibility. (c) Did not build a data-staleness alert — offered to Bastian instead, since the keepalive addresses the actual root cause and a monitor is a separate class of defence.
- **Follow-ups shipped same session (both approved):** (1) `supabase/check_data_freshness.py` + `.github/workflows/data-freshness-check.yml` — weekly Monday check that fails red when the newest scraped event is >14d old or a recent card is only partly scraped; verified it exits 1 with a diagnostic runbook when forced to trip (a check never seen to fail is worthless). (2) `_strip_suffix()` in `_names_match` — drops trailing `jr/sr/ii/iii/iv`; fixed the live "Sean King III" (ufcstats) vs "Sean King" (ESPN) miss, taking Noche UFC from 12/13 to **13/13 fights linked to ESPN**. 18/18 name assertions pass.
- **Verified after Bastian re-enabled:** both workflows `state: active`; `poll-live-fights` pg_cron never broke (active, every minute, succeeding).
- **Known residual (not fixable retroactively):** tonight's Hooker vs Parnasse fights all have `espn_competition_id = NULL` — Phase 5 step 4b only links comp ids to `status='upcoming'` fights, and the month-long outage meant every fight was inserted straight to `completed`. So no per-fight live status for that one card; the LIVE badge still works off `start_time` + the 8h safety net. Self-corrects at Noche UFC (9/12), which is fully linked.
- **NextSteps:** Resume Phase C — S-P2-10, S-P2-12, S-P2-14, then S-P2-19. Also overdue: `/memory-refresh` (`Last refreshed: 2026-05-16`). Candidate for `~/bastian-global-brain/LESSONS.md`: the GitHub 60-day scheduled-workflow disable is a universal fact, not project-specific.
