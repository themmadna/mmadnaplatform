...
- **NextSteps:** (1) **Blocked on ufcstats** — ran `--live` 2026-05-30, found 0 (ufcstats hadn't posted this event's completed results; my manual run finding nothing means it's ufcstats lag, not broken automation). The every-25-min `--live` Action will pull the 3 real bouts (Matthews/Harris, Tsuruya/Gurule, Zhu/Vera) + flip statuses once ufcstats posts (within its 2-day UTC window; manual re-run needed if ufcstats is slower). (2) `supabase/cleanup_song_figueiredo_swaps.py` is **written + tested** — pre-flight aborts (exit 2) until the real bouts land, then deletes the 3 stale rows (8835/8840/8842, verified 0 user data). Re-run it after the next successful scrape. (3) No automation bug found — both the LIVE-badge and phantom-fight display issues are fixed and live in prod regardless of the data lag.
