...
const FingerprintChart = ({ fp, label }) => {
  ...
  const radarData = Object.entries(STAT_LABELS).map(([key, lbl]) => ({
    stat: lbl,
    value: Math.round((fp[key] || 0) * 100),
    fullMark: 100,
  }));
        ...
        <div className="w-[200px] h-[200px] sm:w-[240px] sm:h-[240px] flex-shrink-0" role="img" aria-label={`Scoring fingerprint radar: ${ranked.map(r => `${r.label} ${Math.round(r.pct * 100)}%`).join(', ')}`}>
          <ResponsiveContainer width="100%" height="100%">
            <RadarChart data={radarData} cx="50%" cy="50%" outerRadius="75%">
              ...
              <PolarAngleAxis
                dataKey="stat"
                tick={{ fill: '#94949e', fontSize: 11, fontFamily: 'inherit' }}
              />
              <Radar
                dataKey="value"
                stroke="#ef4444"
    ...
    <div>
      ...
      {hasMomentum && (
        <div className={`flex items-center gap-4 ${hasChart ? 'pt-3 border-t border-white/[0.06]' : ''}`}>
          ...
          <p className="text-[13px] text-pulse-text-2 leading-relaxed">
            <strong className="text-pulse-text">Momentum bias:</strong> when you gave R1 and R2 to the same fighter, you continued in R3 <strong className="text-pulse-text">{momPct}%</strong> of the time.
          </p>
        </div>
      )}
    </div>
