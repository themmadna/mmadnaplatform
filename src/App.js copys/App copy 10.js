...
import { ThumbsUp, ThumbsDown, ChevronLeft, User, Palette, MapPin, Search, X, Activity, Swords, Zap, Target } from 'lucide-react';
import { supabase } from './supabaseClient';
import { dataService } from './dataService';
import LoginPage from './Login';

// --- NEW: Combat DNA Card Component (Auto-Updating Baselines) ---
const CombatDNACard = ({ dna, currentTheme }) => {
  // Default fallbacks (in case DB fetch fails or is loading)
  const [baselines, setBaselines] = useState({
    strikePace: 15.7,       
    grapplingIntensity: 5.6,
    submissionDanger: 0.8,  
    legKickBias: 9.8,       
    groundWarBias: 17.4     
  });

  // Fetch real baselines from Supabase on mount
  useEffect(() => {
    const loadBaselines = async () => {
      const realStats = await dataService.getGlobalBaselines();
      if (realStats) {
        setBaselines(realStats);
      }
    };
    loadBaselines();
  }, []);
  ...
  // Helper to render the "vs Avg" text
  const Comparison = ({ userVal, baseVal, suffix = '' }) => {
    // Safety check for undefined values
    const safeUser = userVal || 0;
    const safeBase = baseVal || 0;
    ...
    return (
      <span className={`text-xs font-bold ml-2 ${isHigher ? currentTheme.accent : 'opacity-40'}`}>
        {isHigher ? '↑' : '↓'} {sign}{diff}{suffix} vs avg
      </span>
    );
  };
    ...
    <div className={`${currentTheme.card} p-6 rounded-2xl border mb-8 shadow-xl animate-in fade-in slide-in-from-bottom-4 duration-700`}>
      <div className="flex items-center gap-3 mb-6 border-b border-white/10 pb-4">
        ...
        <div>
          ...
          <p className="text-xs opacity-50">Compared to UFC Average</p>
        </div>
      </div>

      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="p-4 rounded-xl bg-white/5 border border-white/5 text-center relative overflow-hidden group">
          ...
          <p className="text-xs opacity-50 mb-2">sig. strikes / min</p>
        ...
        <div className="p-4 rounded-xl bg-white/5 border border-white/5 text-center relative overflow-hidden group">
          <p className="text-xs opacity-50 uppercase tracking-widest mb-1">Grappling</p>
          <div className="text-3xl font-black mb-1">{dna.grapplingIntensity}</div>
          <p className="text-xs opacity-50 mb-2">TDs / fight</p>
          <div className="bg-black/20 py-1 px-2 rounded-lg inline-block">
            <Comparison userVal={dna.grapplingIntensity} baseVal={baselines.grapplingIntensity} />
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        <div>
          <div className="flex justify-between text-sm mb-2 font-bold">
            <span className="flex items-center gap-2"><Target size={14} /> Leg Kick Bias (% of strikes that are leg kicks)</span>
            <div className="flex items-center">
              <span>{dna.legKickBias}%</span>
              <Comparison userVal={dna.legKickBias} baseVal={baselines.legKickBias} suffix="%" />
            </div>
          </div>
          <div className="h-2 bg-gray-700 rounded-full overflow-hidden relative">
            {/* Baseline Marker */}
            <div className="absolute top-0 bottom-0 w-0.5 bg-white/30 z-10" style={{ left: `${baselines.legKickBias}%` }} title="UFC Average"></div>
            <div className={`h-full ${currentTheme.primary} transition-all duration-1000`} style={{ width: `${dna.legKickBias}%` }}></div>
          </div>
        </div>

        <div>
          <div className="flex justify-between text-sm mb-2 font-bold">
            <span className="flex items-center gap-2"><Swords size={14} /> Ground War Bias (% of strikes that are ground strikes)</span>
            <div className="flex items-center">
              <span>{dna.groundWarBias}%</span>
              <Comparison userVal={dna.groundWarBias} baseVal={baselines.groundWarBias} suffix="%" />
            </div>
          </div>
          <div className="h-2 bg-gray-700 rounded-full overflow-hidden relative">
             {/* Baseline Marker */}
             <div className="absolute top-0 bottom-0 w-0.5 bg-white/30 z-10" style={{ left: `${baselines.groundWarBias}%` }} title="UFC Average"></div>
            <div className={`h-full ${currentTheme.primary} transition-all duration-1000`} style={{ width: `${dna.groundWarBias}%` }}></div>
          </div>
        </div>

        <div>
          <div className="flex justify-between text-sm mb-2 font-bold">
            <span className="flex items-center gap-2"><Zap size={14} /> Submission Danger (Avg Sub attempts per fight)</span>
            <div className="flex items-center">
              <span>{dna.submissionDanger}</span>
              <Comparison userVal={dna.submissionDanger} baseVal={baselines.submissionDanger} />
            </div>
          </div>
          <div className="h-2 bg-gray-700 rounded-full overflow-hidden">
            <div className={`h-full ${currentTheme.primary} transition-all duration-1000`} style={{ width: `${Math.min(dna.submissionDanger * 33, 100)}%` }}></div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className={`${currentTheme.card} rounded-xl overflow-hidden border mb-6 shadow-lg transition-all`}>
      ...
      <div className="p-6">
        ...
        <div className="h-1.5 w-full bg-gray-700 rounded-full mb-6 relative overflow-hidden">
          <div 
            className={`h-full ${currentTheme.primary} transition-all duration-500 absolute left-0 top-0`} 
...
export default function UFCFightRating() {
  ...
  useEffect(() => {
    const delayDebounce = setTimeout(async () => {
      ...
      const { data: fightMatches } = await supabase
        .from('fights')
        .select(`*, fight_ratings (likes_count, dislikes_count)`)
  ...
  const handleVote = async (fightId, clickedType) => {
    ...
    // --- SPECIAL PROFILE LOGIC ---
    // If we are on the profile, or if we just liked/disliked something, 
    // we need to update userHistory AND re-calculate DNA immediately.
      ...
      <div className="max-w-2xl mx-auto">
        ...
        {currentView === 'profile' && (
          <div className="animate-in slide-in-from-right">
            ...
            {/* NEW: Combat DNA Card - Now updates instantly */}
