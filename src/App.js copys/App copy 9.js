...
const FightCard = ({ fight, currentTheme, handleVote, showEvent = false }) => {
  // Use the merged counts from fight_ratings
...
export default function UFCFightRating() {
  ...
  // 2. SEARCH LOGIC (Updated to join fight_ratings)
  useEffect(() => {
    const delayDebounce = setTimeout(async () => {
      ...
      // Fetch fights matching bout OR event name
      // IMPORTANT: We now select fight_ratings data using the foreign key relationship
      const { data: fightMatches } = await supabase
        .from('fights')
        .select(`
          *,
          fight_ratings (likes_count, dislikes_count)
        `)
      ...
      if (fightMatches && fightMatches.length > 0 && session) {
        ...
        let merged = fightMatches.map(f => ({
          ...f,
          ratings: f.fight_ratings || { likes_count: 0, dislikes_count: 0 }, // Map the joined data
  ...
  // 3. Event Click (Updated to join fight_ratings)
  const handleEventClick = async (event) => {
    setSelectedEvent(event);
    setCurrentView('fights');
    setEventFights([]); 
    
    // Join fight_ratings here too!
  ...
  // 4. Vote Handler (Updates ratings object)
    ...
    const updateList = (list) => list.map(f => {
      if (f.id === fightId) {
        ...
        return { 
          ...f, 
          userVote: finalVote, 
          ratings: { likes_count, dislikes_count } // Update nested ratings object
        };
      }
      return f;
    });
  ...
  // 5. Profile Fetch (Updated to join fight_ratings)
