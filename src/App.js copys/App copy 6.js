  ...
  return (
    <div className={`${currentTheme.card} rounded-xl overflow-hidden border mb-6 shadow-lg transition-all`}>
      <div className="p-4 bg-black/20 text-center">
        ...
        {/* Simple logic: If showEvent is true, use the fight's event_name property */}
...
export default function UFCFightRating() {
  ...
  const fetchUserHistory = async () => {
    ...
    // 1. Get the user's votes
    const { data: votes } = await supabase.from('user_votes')
      .select('fight_id, vote_type')
    ...
    // 2. Fetch the corresponding fights
    ...
    // 3. Fetch all event dates to match them up
