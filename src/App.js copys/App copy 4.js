...
// likes and dislikes how up in profile
// database likes and dsilikes not working properly
...
const FightCard = ({ fight, currentTheme, handleVote }) => {
  ...
  // Maps to your DB column "bout" and handles naming
...
export default function UFCFightRating() {
  ...
  // Fixed: Fetches only events for the selected year
  useEffect(() => {
  ...
  // Fetches fights for the specific event clicked
  const handleEventClick = async (event) => {
    setSelectedEvent(event);
    setCurrentView('fights');
    setEventFights([]); // Clear while loading
  ...
  const handleVote = async (fightId, voteType) => {
    const listToUpdate = currentView === 'profile' ? userHistory : eventFights;
    const fight = listToUpdate.find(f => f.id === fightId);
    if (!fight) return;
    
    const newVote = fight.userVote === voteType ? null : voteType;

    const updateState = (prev) => prev.map(f => {
      if (f.id === fightId) {
        let { likes_count, dislikes_count } = f;
        if (f.userVote === 'like') likes_count--;
        if (f.userVote === 'dislike') dislikes_count--;
        if (newVote === 'like') likes_count++;
        if (newVote === 'dislike') dislikes_count++;
        return { ...f, userVote: newVote, likes_count, dislikes_count };
      }
      return f;
    });
    ...
    try {
      await dataService.castVote(fightId, newVote);
        ...
        {currentView === 'events' && (
          <>
            <div className="flex gap-2 mb-8 overflow-x-auto pb-2 scrollbar-hide">
              {availableYears.map(y => (
                <button key={y} onClick={() => setSelectedYear(y)} 
                  className={`px-6 py-2 rounded-full font-bold border transition-all ${selectedYear === y ? `${currentTheme.primary} border-transparent text-white` : 'border-white/10 opacity-50'}`}>
                  {y}
                </button>
              ))}
            </div>
            <div className="grid gap-4">
              {fetchingEvents ? (
                <p className="text-center opacity-40 py-10">Loading events for {selectedYear}...</p>
              ) : events.length > 0 ? (
                events.map(event => (
          ...
          <div className="animate-in slide-in-from-right">
            ...
            <div className="space-y-4">
              {userHistory.filter(f => f.userVote === activeProfileTab).length > 0 ? (
                ...
                <p className="text-center opacity-40 italic py-20">No {activeProfileTab}d bouts found.</p>
              )}
            </div>
            
            <button onClick={() => supabase.auth.signOut()} className="w-full mt-12 py-4 bg-red-600/10 text-red-500 border border-red-500/50 rounded-xl font-bold hover:bg-red-600 hover:text-white transition-all">SIGN OUT</button>
          </div>
        )}
      </div>
    </div>
