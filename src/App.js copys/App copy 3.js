...
// Defaults to most recent year with an event
// Has theme selection
...
export default function UFCFightRating() {
  ...
  const [selectedYear, setSelectedYear] = useState(''); // Start empty, filled by fetchYears
  ...
  useEffect(() => {
    const initApp = async () => {
      ...
      await fetchYears(); // This now sets the default year
  ...
  const fetchYears = async () => {
    ...
    if (data) {
      ...
      // Default to the newest year available in the DB
