...
// stats are only based on fighter and not fight total
...
export const dataService = {
  ...
  async getCombatDNA(likedFights) {
    ...
    const fightAggregates = likedFights.map(likedFight => {
      ...
      // Skip if we don't have stats or meta for this fight
      if (matchStats.length === 0 || !matchMeta || !matchMeta.time) return null;
      ...
      // Exact time calculation matches your SQL logic: ((R-1)*5) + mins + (secs/60)
      const exactFightMinutes = ((endRound - 1) * 5) + mins + (secs / 60);
      if (exactFightMinutes <= 0) return null;

      // Note: Using sig_strikes_attempted to match the SQL pace baseline
    ...
    const totalSigAnalyzed = fightAggregates.reduce((sum, f) => sum + f.totalSigStrikes, 0);
    ...
    return {
      // Strike Pace is Significant Strikes per Minute
      strikePace: totalMinutesAnalyzed > 0 ? (totalSigAnalyzed / totalMinutesAnalyzed).toFixed(1) : 0,
      
      // Grappling & Subs use Median (per fight)
      grapplingIntensity: calculateMedian(takedownCounts).toFixed(1),
      submissionDanger: calculateMedian(subCounts).toFixed(1),
      
      // Biases (Percentage of total significant strikes)
      legKickBias: Math.round((totalLegs / totalSigAnalyzed || 0) * 100),
      groundWarBias: Math.round((totalGround / totalSigAnalyzed || 0) * 100),
    };
  }, // <--- Comma added here to continue the object
