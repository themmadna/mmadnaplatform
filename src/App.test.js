...
import { ThumbsUp, ThumbsDown, ChevronLeft, User, Palette, MapPin } from 'lucide-react';
...
export default function UFCFightRating() {
  ...
  const handleVote = async (fightId, clickedType) => {
    ...
    try {
      if (isSwitching) {
        await dataService.castVote(fightId, null);
      }
  ...
  const handleSignOut = async () => {
    ...
    setSession(null); // Explicitly clear session to trigger LoginPage
    setCurrentView('events'); // Reset view for next login
  };
      ...
      <div className="max-w-2xl mx-auto">
                  ...
                  <button key={event.id} onClick={() => handleEventClick(event)} className={`${currentTheme.card} p-6 rounded-xl border text-left hover:scale-[1.01] transition-transform`}>
                    ...
                    <div className="flex flex-col gap-1 mt-1 opacity-50 text-sm">
                      ...
                      {event.event_location && (
                        <p className="flex items-center gap-1 italic"><MapPin size={12} /> {event.event_location}</p>
                      )}
                    </div>
                  </button>
                ))
        ...
        {currentView === 'fights' && (
          <div className="animate-in fade-in">
            ...
            <p className="text-sm opacity-50 mb-8 pl-5 italic">
              {selectedEvent?.event_date} {selectedEvent?.event_location ? `• ${selectedEvent.event_location}` : ''}
            </p>
        ...
        {currentView === 'profile' && (
          <div className="animate-in slide-in-from-right">
            ...
            {/* Using the new handleSignOut function */}
